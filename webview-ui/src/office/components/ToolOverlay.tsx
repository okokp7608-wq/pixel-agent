...
import type { AgentChatMessage, SubagentCharacter } from '../../hooks/useExtensionMessages.js';
  ...
  return (
    <>
      {allIds.map((id) => {
        ...
        const latestChat = [...chatMessages]
          .reverse()
          .find((message) => message.id === id && Date.now() - message.timestamp < 12_000);

        // Only show for hovered or selected agents (unless always-show is on)
        if (!alwaysShowOverlay && !isSelected && !isHovered && !latestChat) return null;
        ...
        if (isDone && !isSelected && !isHovered && !latestChat) {
          return (
            <div
              key={id}
              className="absolute"
        ...
        return (
          ...
          >
            {latestChat && (
              <div className="mb-3 max-w-72 border-2 border-border bg-bg px-5 py-4 shadow-pixel text-sm text-text whitespace-normal leading-snug">
                <span className="block text-2xs text-accent-bright mb-1">
                  {latestChat.sender ?? `Agent #${id}`}
                </span>
                {latestChat.text}
              </div>
            )}
