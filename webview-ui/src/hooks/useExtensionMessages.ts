      ...
      if (msg.type === 'layoutLoaded') {
        ...
        for (const id of incoming) {
          ...
          if (layoutReadyRef.current) {
            os.addAgent(
              restoredAgent.id,
              restoredAgent.palette,
              restoredAgent.hueShift,
              restoredAgent.seatId,
              true,
              restoredAgent.folderName,
            );
            addedToLoadedLayout = true;
          } else {
            // Buffer agents until layoutLoaded builds the room and seats.
            pendingAgents.push(restoredAgent);
          }
        }
        if (addedToLoadedLayout) {
          saveAgentSeats(os);
        }
        setAgents((prev) => {
        ...
        if (typeof msg.openRouterApiKeyConfigured === 'boolean') {
          setOpenRouterApiKeyConfigured(msg.openRouterApiKeyConfigured as boolean);
        }
        ...
        setOpenRouterApiKeyConfigured(msg.configured === true);
        ...
        const chatMessage: AgentChatMessage = {
          ...
          role: msg.role as AgentChatMessage['role'],
          sender: typeof msg.sender === 'string' ? msg.sender : undefined,
          text: String(msg.text ?? ''),
          timestamp: typeof msg.timestamp === 'number' ? msg.timestamp : Date.now(),
        };
        setChatMessages((prev) => [...prev.slice(-79), chatMessage]);
      }
    };
