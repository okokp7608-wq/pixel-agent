import { fallbackDecision, parseDecision, restLog, toOfficeTool } from './actions.js';
import type { AgentConfig, DriverConfig } from './config.js';
import { getApiKey } from './config.js';
import type { DriverLogger } from './logger.js';
import type { OpenRouterClient } from './openrouter.js';
import type { DriverSession, OfficeBridge } from './office.js';
...
export class AgentRunner {
  ...
  constructor(
    ...
    private readonly office: OfficeBridge,
  ...
  async run(signal: AbortSignal): Promise<void> {
    ...
    this.logger.agent(
      this.agent.name,
      `[${this.agent.name}] 시작: ${this.agent.model} 모델로 오피스에 입장했어요.`,
    );
    ...
    while (!signal.aborted && this.shouldContinue()) {
      ...
      this.office.appendAssistantToolUse(
        this.session,
        toolId,
        officeTool.toolName,
        officeTool.toolInput,
      );
    ...
    if (this.session) {
      await this.office.endSession(this.session, 'exit').catch((error: unknown) => {
        this.logger.warn(
          `[${this.agent.name}] 종료 신호 전송 실패: ${
  ...
  private async decide(apiKey: string | undefined, signal: AbortSignal) {
    this.iteration += 1;
  ...
  private actionDurationMs(): number {
...
function buildDecisionPrompt(agent: AgentConfig, iteration: number): string {
...
function fallbackMovingDecision(agent: AgentConfig, iteration: number) {
  const actions = ['read', 'write', 'run'] as const;
